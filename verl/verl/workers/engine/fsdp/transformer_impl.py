...
from verl.utils.torch_functional import logprobs_from_conf, logprobs_from_logits
    ...
    def prepare_model_outputs(self, output, output_args, micro_batch: TensorDict, logits_processor_func):
        ...
        if use_remove_padding:
            ...
            if pad_mode == DatasetPadMode.NO_PADDING:
                ...
                # conf_log_probs for calibration loss (rmpad path)
                _conf_positions = micro_batch.get("conf_positions", None)
                if _conf_positions is not None and isinstance(_conf_positions, torch.Tensor) and not self.use_ulysses_sp:
                    _resp_mask = micro_batch.get("response_mask", None)
                    if _resp_mask is not None:
                        _resp_lens = _resp_mask.float().sum(dim=1).long().to(cu_seqlens.device)
                        _prompt_lens = cu_seqlens.diff().long() - _resp_lens
                        _valid = _conf_positions >= 0
                        _conf_flat = torch.where(
                            _valid.to(cu_seqlens.device),
                            cu_seqlens[:-1].long() + _prompt_lens + _conf_positions.long().to(cu_seqlens.device) - 1,
                            torch.full_like(_conf_positions, -1, dtype=torch.long).to(cu_seqlens.device),
                        )
                        model_output["conf_log_probs"] = logprobs_from_conf(logits_rmpad, _conf_flat)
            else:
                raise NotImplementedError(f"pad_mode {pad_mode} not implemented")
